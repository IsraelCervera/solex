...
<div id="product-list">
          ...
          <div class="product-item col-md-6 pull-left panel panel-default">
           ...
           <div class="data">
                  <h3 class="product-name"><a class="text-light" href="{{ route('products.show', $product->id) }}">{{ $product->name }}</a></h3>
                  <p class="datos text-light">Categoria: {{$product->category->name}}</p>
                  <p class="datos text-light">Stock: {{$product->stock}}</p>
                  <p class="datos text-light">Precio:$MX {{$product->sell_price}}</p>
                  <br>
          ...
          <button class="btn btn-success" onclick="location.href='{{ route('products.show', $product->id) }}'">Ver</button>
            <button class="btn btn-info" onclick="location.href='{{ route('products.edit', $product->id) }}'">
            Editar</button>
